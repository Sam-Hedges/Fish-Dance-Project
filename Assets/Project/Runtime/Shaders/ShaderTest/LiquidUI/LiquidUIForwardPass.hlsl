...
float4 Fragment(Interpolators input) : SV_TARGET
{
    ...
    float2 uv1 = round(uv * _PixelSize) / _PixelSize;
    uv = uv1 * _Ratio;
    ...
    float simpleNoisePixelated = Remap(round(simpleNoiseMerged * _NoiseRoundFactor ) / _NoiseRoundFactor, float2(0, 1), float2(0, 10));
    ...
    float colourMask = ColourMask(finalColour, float3(0, 0, 0), 0, 0);
    finalColour.rgb = finalColour + colourMask * _BackgroundColour;
    float4 mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, uv);
    finalColour.a = mainTex.a * Remap(colourMask, float2(0, 1), float2(_Colour.a, _BackgroundColour.a));
